...
const numberOfCumLaudes = grades.filter(grade => grade >= 8);

console.log(numberOfCumLaudes.length);
...
const cumLaude = (...[grades]) => {
  const numberOfCumLaudes = grades.filter((grade) => grade >= 8);

  return numberOfCumLaudes.length;
};
...
const averageGrade = (...[grades]) =>
  grades.reduce((partialSum, a) => partialSum + a, 0) / grades.length;
...
const averageGrade2 = (...[grades]) =>
  (grades.reduce((partialSum, a) => partialSum + a, 0) / grades.length).toFixed(
    2
  );
...
let highestGradeBase = 0; // Start cijfer om mee te beginnen

for (let i = 0; i < grades.length; i++) {
  if (grades[i] > highestGradeBase) {
    highestGradeBase = grades[i];
  }
}

console.log('Highest grade: ' + highestGradeBase);
